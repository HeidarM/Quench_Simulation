...
# -----------------------------------------------------------------------------
# Modifications in this file (local hotfix)
#
#   Summary: Fix VQE callback to pass the full parameter vector instead of the
#            first element when using EstimatorV2.
#   Change:  In _get_evaluate_energy(...), replace:
#              for params, value in zip(parameters.reshape(-1, 1), values):
#            with:
#              for params, value in zip(parameters, values):
#   Rationale: The reshape collapsed each row to a single column, so the VQE
#              callback saw only the first parameter. This hotfix restores the
#              documented callback contract: (eval_count, full_params, energy, metadata).
#   Renamed class: VQE → VQE_CALLBACK_BUG_FIX (for local fix).
#   Affected versions: qiskit-algorithms >=  0.4.0 (until upstream fix)
#   Author of modification: <Heidar Moradi / University of Kent>
#   Date: 2025-09-22
#
# This file is derived from Qiskit’s VQE implementation and remains under the
# Apache License, Version 2.0. See the project LICENSE for details.
# -----------------------------------------------------------------------------
...
class VQE_CALLBACK_BUG_FIX(VariationalAlgorithm, MinimumEigensolver):
    ...
    def _get_evaluate_energy(
        ...
        def evaluate_energy(parameters: np.ndarray) -> np.ndarray | float:
            ...
            if self.callback is not None:
                # HEIDAR: THE BUG WAS HERE AND NOW FIXED
                # for params, value in zip(parameters.reshape(-1, 1), values):
                for params, value in zip(parameters, values):
                    eval_count += 1
